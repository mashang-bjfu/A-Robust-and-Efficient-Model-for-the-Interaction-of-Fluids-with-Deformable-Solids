...
public class FluidMain : MonoBehaviour
{
    ...
    /// <summary>
    /// 被激活了的粒子数[与喷射器配套的概念，溃坝的时候是没有的]
    /// </summary>
    private int activeParticleNum;
    ...
    //exp2增加
    private int kernelIDEmitterGPU;
    ...
    public Vector4[] virtualBoundaryParticlesData = null; // 3个物体，那数组长度就是粒子数的3倍.
    ...
    {   
        //exp2增加
        kernelIDEmitterGPU = computeShaderPBFSolver.FindKernel("EmitterGPU");
    ...
    void InitComputeShaderSetValues()
    {
        ...
        computeShaderPBFSolver.SetFloat("_deltaTime", 1.0f / 20.0f);
    ...
    /// <summary>
    /// 把所有粒子初始化为未激活状态，把位置放到天涯海角，为发射器发射粒子作铺垫.
    /// </summary>
    /// 

    //exp2增加
    void InitPositionsEmitter(){
       for (int i = 0; i < particleNum; i++)
        {
            positions[i] = new Vector3(0,-10,0); // 看不见的地方，就等于是天涯海角了。
        }
    }
    ...
    void InitComputeShaderSetBuffers()//两个步骤：01设置buffer，02逐kernel绑定相关buffer
    {
        ...
        // }
        //exp2增加
        InitPositionsEmitter(); // 初始化粒子到天涯海角，供喷射器使用
        ...
        //计算相关的
        //exp2增加
        computeShaderPBFSolver.SetBuffer(kernelIDEmitterGPU, "_positions", _positions);
        computeShaderPBFSolver.SetBuffer(kernelIDEmitterGPU, "_velocities", _velocities);
    ...
    // CPU层面的粒子发射，累加一下就可以了，作用是控制计算着色器线程组数.
    //exp2增加
    void EmitterCPU(){
        if(activeParticleNum >= particleNum)
            return;

        int deltaNum = 32;

        computeShaderPBFSolver.SetInt("_activeParticleNums", activeParticleNum);

        computeShaderPBFSolver.Dispatch(kernelIDEmitterGPU, deltaNum / 32, 1, 1);

        activeParticleNum += 32;
        computeShaderPBFSolver.SetInt("_activeParticleNums", activeParticleNum);
    ...
    void Update()
    {
        ...
        //仿真计算
        computeShaderPBFSolver.Dispatch(kernelIDPreTreatment, activeParticleNum/*particleNum*/ / 32, 1, 1);
        //构造邻居列表，加速计算
        computeShaderPBFSolver.Dispatch(kernelIDNeighbourSearchReset, gridScale[0] * gridScale[1] * gridScale[2] / 32, 1, 1);
        computeShaderPBFSolver.Dispatch(kernelIDNeighbourSearchInit, activeParticleNum/*particleNum*/ / 32, 1, 1);
        computeShaderPBFSolver.Dispatch(kernelIDComputeNeighbourList, activeParticleNum/*particleNum*/ / 32, 1, 1);
        //构造结束
        for (int i = 0; i < 5; ++i)
        {
            // 准备碰撞检测
            GetComponent<ModelsMain>().ComputeVolumeAndPositionForBoundaryParticles(activeParticleNum/*particleNum*/);
            // 仿真
            computeShaderPBFSolver.Dispatch(kernelIDComputeLambdas, activeParticleNum/*particleNum*/ / 32, 1, 1);
            computeShaderPBFSolver.Dispatch(kernelIDComputeDeltaPositions, activeParticleNum/*particleNum*/ / 32, 1, 1);
            computeShaderPBFSolver.Dispatch(kernelIDUpdatePositions, activeParticleNum/*particleNum*/ / 32, 1, 1);
        }
        computeShaderPBFSolver.Dispatch(kernelIDPostTreatment, activeParticleNum/*particleNum*/ / 32, 1, 1);
    ...
    /// <summary>
    /// 在Unity中，OnRenderObject是一个重要的函数，它在摄像机完成所有常规场景渲染后调用。
    /// 这意味着，在场景中的其他对象已经被渲染后，OnRenderObject将被触发。
    /// OnRenderObject的主要用途是允许开发者通过Graphics.DrawMeshNow或其他函数来绘制自己的对象或自定义几何图形。
    /// 这使得开发者能够在常规的渲染过程之后，添加额外的渲染步骤或效果。
    /// 与OnPostRender类似，OnRenderObject也是用于后期渲染处理，但它们的区别在于调用时机和调用对象。
    /// OnPostRender是在相机完成场景渲染后调用，而OnRenderObject是在具有包含该函数的脚本的任何对象上调用，
    /// 无论这些对象是否附加到摄像机。
    /// </summary>
    void OnRenderObject()
    {
    ...
    //2024-05-15，遇到了这里定义的绘制方法无效的问题；解决方法如下：https://blog.csdn.net/RICKShaozhiheng/article/details/52318613
