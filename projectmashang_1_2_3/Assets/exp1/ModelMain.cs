...
public class ModelMain : MonoBehaviour
{
    ...
    //td存储了三角形面片的信息以及计算点到三角形的最近距离，dg存储了aabb盒上网格和网格上node的信息，可计算一点SDF和volume等
    ...
    void Awake(){
        ...
        // 01.生成离散网格的相关数据
        //这个代码挺重要的，说明了TrangleMeshDistance类中需要加入什么东西
        ...
        //这个代码挺重要的，说明了DiscreteGrid类中需要加入什么东西
    ...
    void Update()
    {
        ...
        // 计算deltaVertex
        //这个nowVertices是哪里算出来的？仅仅靠不断地Array.Copy就能实现？仅靠两个语句之间的时间差就能当做新位置和旧位置？
        //仔细看，else中将上一帧的nowVertices放到oldVertices中，再将本帧的mesh.vertices放到nowVertices中，这样就存储了两帧的数据
    ...
    Vector4 GenerateRandomASTU(){
        ...
        float a=1-s-t-u; // a,s,t,u are the barycentric coordinates of the random point.怎么重心坐标是四维的？多出来一个数据是干什么的？
