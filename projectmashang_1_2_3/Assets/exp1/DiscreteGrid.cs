...
// 尝试把这些单独抽出来，这样看着更清晰
//这个类是模拟什么的？
//该脚本给出网格和网格上节点的相关处理方法，包括根据x位置对SDF和体积场进行差值计算等
//下面所说的网格是指cell，是aabb盒中的网格
public class DiscreteGrid
{
    ...
    //Bounds是包围盒，之后的domain.min和domain.max分别是包围盒的最大点和最小点，就是包围盒的右上后点和左下前点
    public Bounds domain;
    public Vector3Int resolution;
    //TriangleMeshDistaince类记录了：与一点处最近的固体点，最近的三角形，距离。以及求距离的方法等
    public TriangleMeshDistance td = null; // 一层套一层，有内味儿了……

    public Vector3 cellSize;
    public int numberOfCells;
    //nv是所有网格上顶点的总和（这里做个区分，顶点是指正方形四个角的点，节点Node才包括边上的点），ne_x应该是平行于x方向的边的数量
    public int nv, ne_x, ne_y, ne_z,ne;
    public int numberOfNodes;

    //这个也许是网格上节点的位置，SDF值和volume值
    ...
    //给出AABB盒范围内的 cell的顶点数量，边数量，node数量
    void InitValues()
    {
    ...
    /// 并根据三维序号，计算该node在模型局部坐标系下的坐标result
    /// </summary>
    ...
    //计算出各个nodeLocalPosition（各node的坐标）cells（cell上node的编号），各node的SDF,各node的volumeNode
    void InitArrays()
    {
        ...
        /// <returns></returns>
        /// //计算体积图
        /// index应该是node的编号，sampleBasic对应插值时用到的32个node的32个权重
